...
async function run() {
    ...
    for (const row of rows) {
        console.log(`${row[dataColumns[0]]} "${row[dataColumns[3]]}" ${row[dataColumns[2]]} ${row[dataColumns[1]]}yo died in ${row[dataColumns[4]]} of ${row[dataColumns[6]]} ${row[dataColumns[5]] && row[dataColumns[5]] !== 'null'
            ? '(' + row[dataColumns[5]] + ')'
            : ''}`);
    }
}
